...
class FareUpsellEnv(object):
  ...
  def reset(self):
    ...
    opt_rev_ind = np.argmax(self.revenue, axis=None)
    print("Optimal Revenue Is: {}".format(self.revenue.ravel()[opt_rev_ind]))
    y_,x_ = np.unravel_index(opt_rev_ind,self.revenue.shape)
    print("optimal prices are: {},{}".format(self.x[x_],self.y[y_]))
    return None
